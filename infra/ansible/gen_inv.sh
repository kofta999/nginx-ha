...
monitoring_public_ip="$(printf '%s' "${tf_json}" | jq -r '.monitoring_public_ip.value // empty')"
...
# Jump host: use monitoring public IP (stable admin entrypoint)
if [ -n "${monitoring_public_ip}" ] && [ "${monitoring_public_ip}" != "null" ]; then
  jump_ip="${monitoring_public_ip}"
else
